...
class BlackjackMDP(mdp.MDP):
    ...
    def succAndProbReward(self, state: Tuple, action: str) -> List[Tuple]:
        ...
        (hand_value, peek_index, card_count) = state

        if card_count is None:  # End of the game.
            return []

        total_cards = sum(card_count)
        if action == 'Quit':
            return [((hand_value, peek_index, None), 1.0, hand_value)]

        elif action == 'Take':
            if peek_index is not None:
                card_count_list = list(card_count)
                card_count_list[peek_index] -= 1
                if hand_value + self.cardValues[peek_index] > self.threshold: # Bust
                    return [((hand_value + self.cardValues[peek_index], None, None), 1.0, 0)]
                else:
                    return [((hand_value + self.cardValues[peek_index], None, tuple(card_count_list)), 1.0, 0)]
            
            result = []
            for idx, count in enumerate(card_count):
                if count > 0:
                    card_count_list = list(card_count)
                    card_count_list[idx] -= 1
                    if hand_value + self.cardValues[idx] > self.threshold: # Bust
                        state = (hand_value + self.cardValues[idx], None, None)
                        result.append((state, count / total_cards, 0))
                    elif total_cards == 1:  # Draw last card and not bust
                        state = (hand_value + self.cardValues[idx], None, None)
                        result.append((state, count / total_cards, state[0]))
                    else:
                        state = (hand_value + self.cardValues[idx], None, tuple(card_count_list))
                        result.append((state, count / total_cards, 0))
            return result

        elif action == 'Peek':
            result = []
            if peek_index is None:  # Player should not peek twice in a row.
                for idx, count in enumerate(card_count):
                    if count > 0:
                        state = (hand_value, idx, card_count)
                        result.append((state, count / total_cards, -self.peekCost))
            return result
...
class QLearningAlgorithm(mdp.RLAlgorithm):
...
def simulate_QL_over_MDP(mdp: BlackjackMDP, featureExtractor: Callable):
    ...
    mdp.simulate(mdp, ql, numTrials=30000)
...
def blackjackFeatureExtractor(state: Tuple, action: str) -> List[tuple]:
    ...
    if counts is not None:
        bitmask = [0 if count == 0 else 1 for count in counts]
        features.append((('bitmask', tuple(bitmask), action), 1))
...
def compare_changed_MDP(original_mdp: BlackjackMDP, modified_mdp: BlackjackMDP, featureExtractor: Callable):
    ...
    fixed_rl = mdp.FixedRLAlgorithm(v_iter.pi)
    fixed_rl_rewards = mdp.simulate(modified_mdp, fixed_rl, numTrials=30000)
    # run Qlearning on the modified MDP
    q_rl = QLearningAlgorithm(modified_mdp.actions, modified_mdp.discount(), featureExtractor)
    q_rewards = mdp.simulate(modified_mdp, q_rl, numTrials=30000)
...
def sampleKTrajectories(mdp: SeaLevelRiseMDP, val: mdp.ValueIteration):
    invs, wats = 0, 0
    for n in range(1000):
        traj = mdp.sample_trajectory(mdp, val)
...
def sampleKRLTrajectories(mdp: SeaLevelRiseMDP, rl: QLearningAlgorithm):
    invs, wats = 0, 0
    for n in range(1000):
        traj = mdp.sample_RL_trajectory(mdp, rl)
        invs += traj.count('Invest')
        wats += traj.count('Wait')
...
def compare_changed_SeaLevelMDP(orig_mdp: SeaLevelRiseMDP, modified_mdp: SeaLevelRiseMDP):
    ...
    invs, wats = 0, 0
    for n in range(1000):
        traj = mdp.sample_trajectory(orig_mdp, v_iter)
        invs += traj.count('Invest')
        wats += traj.count('Wait')
    print(f"\nPi of Original MDP -> year:{orig_mdp.startState()[0]}, budget:{orig_mdp.startState() [1]}, infra:{orig_mdp.startState()[2]}, sealevel:{orig_mdp.startState()[3]} n_years:{orig_mdp.n_years}, & failure_cost:{orig_mdp.failure_cost}")
    print(f"  *  total invest states: {invs} total wait states: {wats}")
    ...
    invs, wats = 0, 0
    for n in range(1000):
        traj = mdp.sample_trajectory(modified_mdp, v_mod)
        invs += traj.count('Invest')
        wats += traj.count('Wait')
